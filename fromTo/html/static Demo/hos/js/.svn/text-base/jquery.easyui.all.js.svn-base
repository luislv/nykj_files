...
(function($){
	...
	function createWindow(options){	
		//var w = (window.innerWidth - 250)/2;     //zj
		//var h = (window.innerHeight - 100)/2;    //zj
		var w = (screen.Width - 250)/2;     //zj
		var h = (screen.Height - 100)/2;    //zj
		//var w = ($(document.body).width() - 250)/2;     //zj
		//var h = ($(document.body).height() - 100)/2;    //zj
	...
	$.messager = {
		...
		confirmAdd: function(title,msg,btn1,btn2,fn) {
			var content = '<div class="messager-icon messager-question"></div>'
					+ '<div class="mt20">' + msg + '</div>'
					+ '<div style="clear:both;"/>';
			var buttons = {};
			buttons[btn1] = function(){
				win.window('close');
				if (fn){
					fn(true);
					return false;
...
(function($){
	...
	function create(target){
		...
		tree.tree($.extend({}, opts, {
			...
			onLoadSuccess: function(node, data){
				if (opts.multiple){
					...
					if(value == '') {
						var selectNodeId = getFirstSelectedNode(data);
						if(selectNodeId != '') {
							value = selectNodeId;
						}
					}
		...
		function getFirstSelectedNode(children) {
			for(var i=0; i<children.length; i++) {
				var c = children[i];
				var id = c['id'];
				if (c['selected']){
					...
					return getFirstSelectedNode(c.children);
				} else {
					return '';
				}
			}
